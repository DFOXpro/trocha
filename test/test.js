(function() {
  var _clone, base_variables, constants_test, constructor_test, function_path_test, issues_test, route_types_test, routes_creation_test, test, testFramework;
  ...
  routes_creation_test = function() {
    describe('Route creation', function() {
      ...
      it('should create routes via post init functions', function() {
        var r;
        r = new Trocha();
        r._newRoute({
          name: "simple_route"
      ...
      return describe('Route creation params', function() {
  ...
  issues_test = function() {
    describe('Solve issues', function() {
      it('1: alias routes must provide a path funtion', function() {
        ...
        assert(myRoutes.hello.name.path({
          my_name: 'underworld'
        }), 'hi/underworld');
        assert(myRoutes.hello.myCountry.path(), 'hi/Colombia/:department');
        return assert(myRoutes.hello.myCountry.path({
      ...
      return it('4: customSelector must works with routes attributes', function() {
        var myRoutes;
        myRoutes = new Trocha({
          customSelector: 'TRCH',
          routes: {
        ...
        assert(myRoutes.hello.path({
          name: 'World'
        }), '/hello/World');
        assert(myRoutes.hello.TRCHid, 'name');
        assert(myRoutes.hello.$id, {});
        assert(myRoutes.hello.TRCHmethod, 'GET');
        return assert(myRoutes.hello.$id.path(), '/hello/:name/$id');
      });
    });
    return issues_test = void 0;
  };
